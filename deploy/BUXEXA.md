# buxexa — team launcher for the Jota AI Gateway

`buxexa` is a tiny global command that launches Claude Code against the shared
...
Installs `buxexa` to `~/.local/bin/`. If that's not on your `PATH`, the installer
...
1. **Tailscale** — installed and logged in to the **jota.ai** tailnet, and your user
   ...
   the tailnet group that has the grant if `buxexa` reports "not reachable".
...
```sh
buxexa                 # interactive Claude Code via the gateway
buxexa "explain this"  # args pass straight through to claude
```
...
| Var | Default | Meaning |
|-----|---------|---------|
| `BUXEXA_HOST` | `fcc-proxy` | proxy MagicDNS host on the tailnet |
| `BUXEXA_TAILNET` | `tail576af6.ts.net` | tailnet DNS suffix |
| `BUXEXA_PORT` | `8082` | proxy port |
| `BUXEXA_TOKEN` | `freecc` | proxy auth token |
| `BUXEXA_WAIT` | `30` | seconds to wait for the gateway |

Example — point at a differently-named gateway node:

```sh
BUXEXA_HOST=fcc-proxy-staging buxexa
```
Or set it permanently in your `~/.zshrc`:
```sh
export BUXEXA_HOST=fcc-proxy-staging
```
(The staging node is `fcc-proxy`, which is the default — no override needed.)

## How it works

`buxexa` checks you're on the tailnet, waits for the gateway to be reachable at
...
- **"not reachable within 30s"** → `tailscale status` (are you up? is the gateway node
  listed?); confirm the ACL grant; check the host name (`BUXEXA_HOST`).
...
- **401 from the gateway** → token mismatch; the gateway's `ANTHROPIC_AUTH_TOKEN` isn't
  `freecc` — set `BUXEXA_TOKEN` to match.
