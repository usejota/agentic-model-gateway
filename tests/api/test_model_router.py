...
def test_model_router_appends_1m_suffix_for_catalog_model(settings):
    resolved = ModelRouter(settings).resolve("deepseek/deepseek-v4-pro")

    assert resolved.provider_id == "deepseek"
    assert resolved.provider_model == "deepseek-v4-pro[1m]"
    assert resolved.one_m_context is True


def test_model_router_appends_1m_suffix_for_nested_catalog_model(settings):
    resolved = ModelRouter(settings).resolve("open_router/minimax/minimax-m3")

    assert resolved.provider_id == "open_router"
    assert resolved.provider_model == "minimax/minimax-m3[1m]"
    assert resolved.one_m_context is True


def test_model_router_no_1m_suffix_for_non_catalog_model(settings):
    resolved = ModelRouter(settings).resolve("nvidia_nim/meta/llama-3.3-70b")

    assert resolved.provider_id == "nvidia_nim"
    assert resolved.provider_model == "meta/llama-3.3-70b"
    assert resolved.one_m_context is False


def test_model_router_1m_suffix_idempotent_when_already_present(settings):
    resolved = ModelRouter(settings).resolve("deepseek/deepseek-v4-pro[1m]")

    assert resolved.provider_model == "deepseek-v4-pro[1m]"
    assert resolved.one_m_context is True


def test_model_router_decodes_1m_gateway_id_and_appends_suffix(settings):
    routed = ModelRouter(settings).resolve_messages_request(
        MessagesRequest(
            model="claude-3-freecc-1m/open_router/minimax/minimax-m3",
            max_tokens=100,
            messages=[Message(role="user", content="hello")],
        )
    )

    assert routed.request.model == "minimax/minimax-m3[1m]"
    assert routed.resolved.provider_id == "open_router"
    assert routed.resolved.provider_model == "minimax/minimax-m3[1m]"
    assert routed.resolved.one_m_context is True


def test_model_router_1m_gateway_id_preserves_thinking_resolution(settings):
    settings.enable_model_thinking = True

    resolved = ModelRouter(settings).resolve(
        "claude-3-freecc-1m/deepseek/deepseek-v4-pro"
    )
