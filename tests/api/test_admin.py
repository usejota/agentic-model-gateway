...
def test_admin_explicit_env_values_are_editable(monkeypatch, tmp_path):
    ...
    explicit_env.write_text("MODEL_DELEGATE_EXCLUSIONS=a/b,c/d\n", encoding="utf-8")
    ...
    response = _local_client(app).post(
        "/admin/api/config/apply",
        json={"values": {"MODEL_DELEGATE_EXCLUSIONS": "a/b"}},
    )
    assert response.status_code == 200
    body = response.json()
    assert body["applied"] is True
    ...
    managed_text = (tmp_path / ".fcc" / ".env").read_text("utf-8")
    assert "MODEL_DELEGATE_EXCLUSIONS=a/b" in managed_text
...
def test_admin_explicit_same_as_managed_not_locked(monkeypatch, tmp_path):
    ...
    managed_env_path.write_text("MODEL_DELEGATE_EXCLUSIONS=a/b,c/d\n", encoding="utf-8")
    ...
    assert "MODEL_DELEGATE_EXCLUSIONS=a/b" in text
...
def test_admin_process_env_still_locked(monkeypatch, tmp_path):
    ...
    if managed_env.exists():
        ...
        assert "MODEL_DELEGATE_EXCLUSIONS=a/b" not in text
