...
@pytest.mark.asyncio
async def test_fallback_chain_uses_text_fallback_after_old_image_strip() -> None:
    """Old image in history + text-only current turn → primary is text-only
    (images stripped), and a text-only fallback survives the image-aware filter.

    Regression guard: previously the fallback chain read ``has_images`` on the
    ORIGINAL request (which still carried the old image), so it would skip
    text-only fallbacks even though the primary was already image-free after
    stripping. The fix: pass ``routed.request`` (stripped) into
    ``_fallback_candidates`` so the filter reflects what the primary actually sent.
    """
    provider = _RecordingProvider()
    svc = _service(
        _settings(
    ...
    class _FallbackProvider:
        ...
        async def stream_response(
            ...
            *,
            ...
            fallback_calls.append(request.model)
            yield "event: message_start\ndata: {}\n\n"
            yield "[DONE]\n\n"
    ...
    await _drain(result)
    # The text-only fallback (open_router/anthropic/claude-sonnet-4) was used.
    # Without the routed.request fix it would have been skipped because the
    # original request still has the old image in history.
    assert fallback_calls == ["anthropic/claude-sonnet-4"]
